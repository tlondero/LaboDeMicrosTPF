...
static const uint8_t s_buffer1[] = {'T', 'O', 'M', 'I','\r', '\n'};
...
static void FileAccessTask1(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        {
            error = f_open(&g_fileObject, _T("/tincho/alan.txt"), FA_READ);
            if (error)
            {
                if (error == FR_EXIST)
                {
                ...
                {
                    if (f_open(&g_fileObject, _T("/tincho/alan.txt"), (FA_READ | FA_OPEN_EXISTING)) != FR_OK)
                    {
...
*/
            error = f_read(&g_fileObject, read_buffer, sizeof(read_buffer), &bytesWritten);
            if ((error) || (bytesWritten != sizeof(read_buffer)))
            {
...
static void FileAccessTask2(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (xSemaphoreTake(s_fileAccessSemaphore, s_taskSleepTicks) == pdTRUE)
        {
            error = f_open(&g_fileObject, _T("/tincho/alan.txt"), FA_WRITE);
            if (error)
            {
                if (error == FR_EXIST)
                {
                ...
                {
                    if (f_open(&g_fileObject, _T("/tincho/alan.txt"), (FA_WRITE | FA_CREATE_NEW)) != FR_OK)
                    {
