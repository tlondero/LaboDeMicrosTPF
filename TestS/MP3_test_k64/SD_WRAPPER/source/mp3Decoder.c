...
void readID3Tag(void)
{
    ...
    if (has_ID3_tag(mp3File))
    {
        ...
        if (!read_ID3_info(TITLE_ID3, context_data.ID3_data.title, ID3_MAX_FIELD_SIZE, mp3File))
            strcpy(context_data.ID3_data.title, DEFAULT_ID3_FIELD);

        if (!read_ID3_info(ALBUM_ID3, context_data.ID3_data.album, ID3_MAX_FIELD_SIZE, mp3File))
            strcpy(context_data.ID3_data.album, DEFAULT_ID3_FIELD);

        if (!read_ID3_info(ARTIST_ID3, context_data.ID3_data.artist, ID3_MAX_FIELD_SIZE, mp3File))
        ...
        if(!read_ID3_info(TRACK_NUM_ID3, context_data.ID3_data.trackNum, 10, mp3File))
